...
function resolveRuntimeUrl(path) {
	return new URL(path, moduleBaseUrl).toString();
}

const connection = new BareMuxConnection(resolveRuntimeUrl("./bareworker.js"));
...
function getStaticWispOverride() {
	const configured = globalThis.__CELESTIAL_STATIC_WISP__;
	if (typeof configured !== "string") return null;

	const value = configured.trim();
	return /^wss?:\/\//i.test(value) ? value : null;
}

function normalizeWispLocation(value) {
...
const transportOptions = {
	epoxy: resolveRuntimeUrl("./epoxy/index.mjs"),
	libcurl: resolveRuntimeUrl("./curl/index.mjs")
};

//////////////////////////////
///           SW           ///
//////////////////////////////
const stockSW = resolveRuntimeUrl("./ultraworker.js");
...
await import(resolveRuntimeUrl("./violet/violet.bundle.js"));
await import(resolveRuntimeUrl("./violet/violet.config.js"));

await import(resolveRuntimeUrl("./scram/scramjet.all.js"));
const { ScramjetController } = window.$scramjetLoadController();
const scramjet = new ScramjetController({
	files: {
		wasm: resolveRuntimeUrl("./scram/scramjet.wasm.wasm"),
		all: resolveRuntimeUrl("./scram/scramjet.all.js"),
		sync: resolveRuntimeUrl("./scram/scramjet.sync.js"),
	},
	flags: {
		rewriterLogs: false,
...
export function resolveDefaultWispUrl(savedLocation) {
	const staticWispOverride = getStaticWispOverride();
	const configuredLocation = normalizeWispLocation(savedLocation);

	if (staticWispOverride) {
		if (configuredLocation && /^wss?:\/\//i.test(configuredLocation)) {
			return configuredLocation;
		}

		return staticWispOverride;
	}
	...
	const locationPath = configuredLocation || "./wisp/";
	return `${location.protocol === "https:" ? "wss://" : "ws://"}${location.host}${locationPath}`;
}
...
export class Tab {
	...
	handleLoad() {
		...
		if (url === "index.html?type=ai") url = "celestial://ai";
